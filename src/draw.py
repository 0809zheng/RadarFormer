...
previous_position = []
# adjust the attitude of the coordinate system
theta, phi = 3.1415/6, -3.1415/7
should_rotate = False
scale_dx = 800
scale_dy = 800
...
class Plotter3d:
    ...
    def __init__(self, canvas_size, origin=(0.5, 0.5), scale=1):
        ...
        axes = [
            ...
            np.array([[-axis_length/2, -axis_length/2, 0], [-axis_length/2, axis_length/2, 0]], dtype=np.float32),
            np.array([[-axis_length/2, axis_length/2, 0], [-axis_length/2, axis_length/2, axis_length/2-10]], dtype=np.float32),
            np.array([[-axis_length/2, axis_length/2, axis_length/2-10], [axis_length/2, axis_length/2, axis_length/2-10]], dtype=np.float32),]
        step = 20
        for step_id in range(1, axis_length // step):  # add grid
        ...
        for step_id in range(axis_length // step + 1):  # add grid
            if step_id < (axis_length // step + 1)/2:
                axes.append(np.array([[axis_length / 2, -axis_length / 2,  step_id * step],
                                      [axis_length / 2, axis_length / 2,  step_id * step]], dtype=np.float32))
            axes.append(np.array([[axis_length / 2, -axis_length / 2 + step_id * step, 0],
                                  [axis_length / 2, -axis_length / 2 + step_id * step, axis_length/2-10]], dtype=np.float32))
        for step_id in range(axis_length // step):  # add grid
            if step_id < (axis_length // step + 1)/2:
                axes.append(np.array([[ -axis_length / 2, axis_length / 2,  step_id * step],
                                      [axis_length / 2, axis_length / 2,  step_id * step]], dtype=np.float32))
            if step_id > 0:
                axes.append(np.array([[-axis_length / 2 + step_id * step, axis_length / 2, 0],
                                      [-axis_length / 2 + step_id * step, axis_length / 2, axis_length/2-10]], dtype=np.float32))
    ...
    def _draw_axes(self, img, R):
        ...
        for axe in axes_2d:
            ...
            axe = axe.astype(int)
            if count<3:
                cv2.line(img, tuple(axe[0]), tuple(axe[1]), (93, 93, 93), 1, cv2.LINE_AA)
            else:
                cv2.line(img, tuple(axe[0]), tuple(axe[1]), (235, 235, 235), 1, cv2.LINE_AA)
    ...
    def _plot_edges(self, img, vertices, edges, R):
        ...
        for edge_vertices in edges_vertices:
            ...
            cv2.circle(img, tuple(edge_vertices[0]), radius=3, color=(0,0,255), thickness=-1)
            cv2.circle(img, tuple(edge_vertices[1]), radius=3, color=(0,0,255), thickness=-1)
            cv2.line(img, tuple(edge_vertices[0]), tuple(edge_vertices[1]), (255, 0, 0), 1, cv2.LINE_AA)
